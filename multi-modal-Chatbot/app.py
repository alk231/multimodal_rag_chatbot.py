...
#     add_to_vectorstore(text_full.strip(), file.name)
...
file = st.sidebar.file_uploader("Upload File", type=["pdf", "txt", "mp3", "m4a", "wav", "docx"])
if file:
    with st.spinner("Processing..."):

        # Reset vector store so only THIS file is used
        st.session_state.vector_store = None
        if os.path.exists(VECTOR_STORE_PATH):
            import shutil
...
if image:
    ...
    st.sidebar.success("🖼️ Image ready for questions!")
...
for msg in st.session_state.messages:
    ...
    st.markdown(f"""
        <div style="display:flex; justify-content:{align}; margin:8px 0;">
            <div style="background:{bubble_bg}; color:{text_color}; padding:12px 16px; 
                        border-radius:18px; max-width:70%; font-size:16px; line-height:1.4;">
                {msg["text"]}
            </div>
        </div>
...
if prompt:
    ...
    # Detect if user actually wants to use the image
    image_keywords = [
        "image",
        "photo",
        "picture",
